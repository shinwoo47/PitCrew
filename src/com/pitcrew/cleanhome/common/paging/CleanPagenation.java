...
public class CleanPagenation {
	
	/* 검색어가 없는 경우 페이징 처리만을 위한 용도 */
	public static CleanCriteria getCleanCriteria(int pageNo, int totalCount, int limit, int buttonAmount) {
		
		return getCleanCriteria(pageNo, totalCount, limit, buttonAmount);
	}
...
//		startPage = (((int)((double) pageNo / buttonAmount + 0.9)) - 1) * buttonAmount + 1;
		...
		SelectCriteria cleanCriteria = new CleanCriteria(pageNo, totalCount, limit, buttonAmount ,maxPage, startPage, endPage, startRow, endRow, searchCondition, searchValue);
